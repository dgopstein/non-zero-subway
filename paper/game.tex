...
\subsubsection{Proximal Dominance}
A point $x$ between two other points, $y_1, y_2$ where $y_1 < y_2$ can never be affected by any point below the lower boundary or above the upper. Since the utility function is defined in terms of nearest neighbor which is in turn defined by absolute distance, any point $z$ which $z \leq y_1$ or $z \geq y_2$ implies $\Delta(x, z) \geq \Delta(x, y_1)$ or $\Delta(x, z) \geq \Delta(x, y_2)$ and therefore $nearest\_neighbor(x, y_1, y_2, z) \leq nearest\_neighbor(x, z)$
